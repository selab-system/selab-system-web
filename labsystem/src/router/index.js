...
//   if (localStorage.getItem('roleid') !== '2' && routes.includes(to.path)) { alert('没有权限') } else {
//     next()
//   }
// })
...
router.beforeEach((to, from, next) => {
  ...
  if (!localStorage.getItem('token') && !routes.includes(to.path)) { alert('请先前往登录'); next('/login') } else {
    next()
  }
})
