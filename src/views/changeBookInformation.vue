...
export default {
  ...
  methods: {
    ...
    async remove() {
      if (this.list.bookId) {
        const res = await Delete(this.list.bookId);
        if(res){
          alert("删除成功");
        }
        else{
  ...
  <div class="continue">
    ...
    <el-descriptions title="书籍信息" class="listshow">
      <el-descriptions-item
        ...
        v-model="list.bookId"
        label="图书id"
      >{{list.bookId}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
        v-model="list.bookName"
        label="书籍名称"
      >{{list.bookName}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
        v-model="list.bookAuthor"
        label="图书作者"
      >{{list.bookAuthor}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
        v-model="list.bookDetails"
        label="图书介绍"
      >{{list.bookDetails}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
        v-model="list.price"
        label="价格"
      >{{list.price}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
        v-model="list.ownerName"
        label="书记拥有者"
      >{{list.ownerName}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
        v-model="list.status"
        label="书记状态"
      >{{list.status}}</el-descriptions-item>
      <el-descriptions-item
        class="listshow"
      ...
      <el-descriptions-item
        ...
        label="修改时间"
      >{{list.updateTime}}</el-descriptions-item>
    </el-descriptions>
    <!-- 分割线 -->
    <el-divider></el-divider>
    <!-- 输入要修改信息 -->
    ...
    <el-form ref="addform" :model="addform" label-width="80px">
      ...
      <el-form-item label="图书介绍"
        ><el-input v-model="addform.bookDetails"></el-input
      ></el-form-item>
      ...
      <el-form-item label="书籍编号"
        ><el-input v-model="addform.bookRef"></el-input
      ></el-form-item>
