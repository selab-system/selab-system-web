      ...
      <div class="nav-left-selections">
        ...
        <div class="nav-selection" @mouseleave="showDropdownU = false" v-if="readRoleId !== 3">
          ...
          <div class="dropdown-content" v-if="this.showDropdownU">
            <router-link to="/modifyPersonalInformation">
              <div class="menuDown">个人信息</div>
            </router-link>
            <router-link to="/departmemtMenber">
              <div class="menuDown">部门信息
              </div>
            </router-link>
          </div>
        </div>
        <div class="nav-selection" @mouseleave="showDropdownE = false" v-if="readRoleId !== 3">
          ...
          <div class="dropdown-content-Long" v-if="this.showDropdownE">
            <router-link to="/enroll">
              <div class="menuDown" v-if="!taskIsShowed">查看报名信息</div>
            </router-link>
            <router-link to="/registrationForm">
              <div class="menuDown" v-if="taskIsShowed">报名</div>
            </router-link>
          </div>
        </div>
        <div class="nav-selection" @mouseleave="showDropdownB = false" v-if="readRoleId !== 3">
          ...
          <div class="dropdown-content" v-if="this.showDropdownB">
            <router-link to="/queryAllBooks">
              <div class="menuDown">查询书籍</div>
            </router-link>
            <router-link to="/personalBooks">
              <div class="menuDown">个人书籍</div>
            </router-link>
            <router-link to="/checkBorrowing">
              <div class="menuDown" v-if="bookRecord">查询借阅记录</div>
            </router-link>
          </div>
        </div>
        <div class="nav-selection dropdown" @mouseleave="showDropdownT = false" v-if="!taskIsShowed">
          ...
          <div class="dropdown-content" v-if="this.showDropdownT">
            <router-link to="/release">
              <div class="menuDown"> 发布任务 </div>
            </router-link>
            <router-link to="/CheckAllTask">
              <div class="menuDown">查看全部任务
              </div>
            </router-link>
          </div>
        </div>
